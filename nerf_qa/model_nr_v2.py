...
class NRModel(nn.Module):
    ...
    def __init__(self, device='cpu', refine_up_depth = 2):
        ...
        self.transformer_decoder = nn.Sequential(*[Block(initial_dists_dim + initial_sem_dim, 8, attn_class=MemEffAttention) for _ in range(wandb.config.transformer_decoder_depth)])
    ...
    def pred_gt_dists_feats(self, dists_feats, dinov2_feats):
        ...
        feature_map = torch.concat([dists_feats[-1], dinov2_feats], dim=1)
        C = feature_map.shape[1]
        trans_decode = self.transformer_decoder(feature_map.reshape(-1, C, 256).permute(0,2,1))
        trans_decode = trans_decode.permute(0,2,1).reshape(-1, C, 16, 16)
        feature_map = feature_map + trans_decode
        predicted_feats = []
