...
parser.add_argument('--eps_scale', type=float, default=1.0, help='Optimizer epsilon.')
...
config = {
    ...
    "kappa": kappa,
    "lr": args.lr_scale * 5e-5 * np.sqrt(kappa),
    "beta1": np.power(args.beta1_scale * 0.6177, kappa),
    "beta2": np.power(args.beta2_scale * 0.9795, kappa),
    "eps": args.eps_scale * 3e-10 / np.sqrt(kappa),
}     
...
wandb.log({
    ...
    "Cross-Validation Metrics Dict/rmse_mean": np.mean(rmses),
    "Cross-Validation Metrics Dict/rmse_std": np.std(rmses),
...
# wandb.log({
#     "Cross-Validation Metrics Dict/rmse_hist": wandb.Histogram(np.array(rmses)),
